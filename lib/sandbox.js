...
const host_fs = host.require('fs');
const host_pa = host.require('path');
...
return ((vm, host) => {
	...
	const BUILTINS = {};
	const CACHE = {};
	const fs = vm.options.fetcher || host_fs;
	const pa = vm.options.fetcher || host_pa;
...
*/
			...
			if (vm.options.require.root) {
				...
				console.log('filename', filename, 'dirname', dirname,'root', requiredPath)
